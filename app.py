...
def delete_customer():
    if request.method == 'POST':
        ...
        # בדיקה השאלות לקוח (לא מוחק במידה ויש השאלה)
...
def delete_book():
    if request.method == 'POST':
        ...
        # בדיקה השאלות לספר (לא מוחק במידה ויש השאלה)
        cursor.execute('SELECT * FROM Loans WHERE bookID = ? AND ActualReturnDate IS NULL', (book_id,))
        existing_loan = cursor.fetchone()
    ...
    if request.method == 'POST':
        if 'customer_name' in request.form:
            ...
            cursor = connection.cursor()
                           ...
                           # קבלת מידע על לקוח לפי שם
            cursor.execute('SELECT * FROM Customers WHERE Name = ?', (name,))
            customer = cursor.fetchone()

            if customer:
                           # בדיקה אם הלקוח השאיל ספרים
            ...
            cursor = connection.cursor()
                             ...
                             #  קבלת מידע על ספר לפי שם

            cursor.execute('SELECT * FROM Books WHERE Name = ?', (name,))
            book = cursor.fetchone()

            if book:
                           # בדיקה אם הספר הושאל ע״י אחד הלקוחות
...
def late_loans():
    ...
    connection = sqlite3.connect('library.db')
    cursor = connection.cursor()
             #  כבדיקת תאריך ההשאלה דרך ממסד הנתונים
